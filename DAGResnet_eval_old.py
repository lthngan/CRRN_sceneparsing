...
def saveLabelResults(images, labels, predictions, filenames):
  ...
  for i in range(nSamples):
    ...
    image_label_overlay = label2rgb(predictions[i], image=image)
    count = count + 1
...
def eval_once(saver, summary_writer, label_predict, summary_op, labels, images, filenames):
  """Run Eval once.
  ...
  with tf.Session() as sess:
    ...
    try:
      ...
      while step < num_iter and not coord.should_stop():
        ...
        predictions, image, label, filename = sess.run([label_predict, images, labels, filenames])
        #image = sess.run(images)
        #label = sess.run(labels)
        saveLabelResults(image, label, predictions, filename)
...
def evaluate(dataset):
  ...
  with tf.Graph().as_default() as g:
    ...
    while True:
      eval_once(saver, summary_writer, label_predict, summary_op, labels, images, filenames)
      if FLAGS.run_once:
        break
